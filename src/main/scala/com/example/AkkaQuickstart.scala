...
case class Quote(date: LocalDate,
                 open: Double,
                 high: Double,
                 low: Double,
                 close: Double,
...
//  println("Hello, World")
  ...
  def loadQuotes(filename: String): List[Quote] = {
    ...
    for (line <- bufferedSource.getLines) {
      val cols = line.split(",").map(_.trim)
      val entry = Quote(LocalDate.parse(cols(0), dtf),
                        cols(1).toDouble,
                        cols(2).toDouble,
                        cols(3).toDouble,
                        cols(3).toDouble,
  ...
  val divList = loadDividends(
    "./src/main/resources/" + ticker + "-dividends.csv")
  ...
  val (_, withQuotes) =
    divList.partition(_.exDate.compareTo(priceList.reverse.head.date) < 0)
  val totalDividends =
    withQuotes.foldLeft(0.0)((left, right) => left + right.amount)
  println(
    "starting investment: " + priceList.reverse.head.close + " dividends: " + totalDividends + " endValue: " + (totalDividends + priceList.head.close))
  println(
    "return: " + (priceList.reverse.head.close) / (totalDividends + priceList.head.close - priceList.reverse.head.close) * 100 + "%")
}
